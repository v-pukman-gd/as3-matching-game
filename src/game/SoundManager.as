package game {
	...
	public class SoundManager {
		...
		// -------------------------------------------------------------------------------------------------------------------------		
		/** Play a sound */
		public function playSound(id:String, volume:Number = 1.0, repetitions:int = 1, loop:Boolean = false, panning:Number = 0):void {			
			...
			if (soundIsAdded(id))
			{
				...
				if(!loop)
					...
					channel.addEventListener(Event.SOUND_COMPLETE,playSoundAgain);
			...
			{
				//trace("sound is not in sound manager");
				//throw Error("The sound you are trying to play (" + id + ") is not in the Sound Manager. Try adding it to the Sound Manager first.");
			}
		}
		
		private function playSoundAgain(e:Event):void 
		{
			for (var id:String in currPlayingSounds) 
			{
				if (currPlayingSounds[id].channel == e.target)
				{
					playSound(id, currPlayingSounds[id].volume, 1, true);
					break;
				}
			}
			e.currentTarget.removeEventListener(Event.SOUND_COMPLETE, playSoundAgain);
		}
		// -------------------------------------------------------------------------------------------------------------------------		
			...
			if (soundIsPlaying(id))
			{
				...
				Starling.juggler.tween(currPlayingSounds[id], tweenDuration, {
					...
					onUpdate: function():void {
						if (!_isMuted && currPlayingSounds[id])
						{
		...
		public function crossFade(fadeOutId:String, fadeInId:String, tweenDuration:Number = 2, fadeInVolume:Number = 1, fadeInRepetitions:int = 1, loop:Boolean = false):void {			
			
			// If the fade-in sound is not already playing, start playing it
			if (!soundIsPlaying(fadeInId))
				playSound(fadeInId, 0, fadeInRepetitions,loop);
